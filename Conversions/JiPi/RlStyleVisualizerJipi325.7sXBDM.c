...
// Connect Buffer A 'https://soundcloud.com/michael_ault/angelwings' to iChannel1


__KERNEL__ void RlStyleVisualizerJipi325Fuse__Buffer_A(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0, sampler2D iChannel1)
{
    ...
    //get frequency data
    float freq = texelFetch( iChannel1, to_int2(tx,0), 0 ).x;
    ...
    if (int(fragCoord.y) == 2) {
        fragColor = to_float4_aw(to_float3(freq),1.0f);
    }
...
__KERNEL__ void RlStyleVisualizerJipi325Fuse(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0)
{
    ...
    float maxSum = 0.0f;
    for (int i = 0; i<9;i++) {
    sum = sum + texelFetch( iChannel0, to_int2(starter+i,2), 0 ).x;
        maxSum = maxSum + texelFetch( iChannel0, to_int2(starter+i,1), 0 ).x;
    }
