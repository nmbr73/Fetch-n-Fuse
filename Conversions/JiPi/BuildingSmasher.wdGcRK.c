...
#define R iResolution
 ...
 {
  return(to_mat2(m.r0.x,m.r1.x, m.r0.y,m.r1.y)); 	                    
 }

 __DEVICE__ inline float determinant(mat2 A)
{
...
#define T(p) texture(iChannel0,  (make_float2(to_int2(mod_f2f2(p,R)))+0.5f)/R)
#define T1(p) texture(iChannel1, (make_float2(to_int2(mod_f2f2(p,R)))+0.5f)/R)
...
__DEVICE__ float2 destimator(float2 dx, float M)
{
    ...
    float2 ds = distribution_size*clamp(1.0f - difd*_fabs(dx), 0.001f, 1.0f);
    return ds + 0.3f*_fmaxf(M/(ds.x*ds.y) - 1.1f, 0.0f)*dt;
}

__DEVICE__ float deformation_energy(mat2 D)
{
    D = mul_mat2_mat2(transpose(D) , D);
...
#define elastic_lambda 3.2f
#define elastic_mu 4.2f
...
__KERNEL__ void BuildingSmasherFuse__Buffer_A(float4 U, float2 pos, int iFrame, sampler2D iChannel2)
{
    ...
    range(i, -1, 1) range(j, -1, 1)
    {
        ...
        float4 aabbX = to_float4(_fmaxf(pos - 0.5f, X0 - K*0.5f), _fminf(pos + 0.5f, X0 + K*0.5f)); //overlap aabb
    ...
    if(iFrame < 1)
    {
        ...
        M = _fmaxf(_fmaxf(Building(X - R*to_float2(0.5f,0.32f), R*to_float2(0.4f,0.3f)),
                          ...
                          particleBox(X - R*to_float2(0.5f,0.12f), R*to_float2(0.47f, 0.1f)));
                    
        C = mod_f(3.0f*pos/R, 1.0f);
    }
    
    X = X - pos;
    U = to_float4(ENCODE(X), ENCODE(V), M, ENCODE(C));
...
__KERNEL__ void BuildingSmasherFuse__Buffer_B(float4 U, float2 pos, int iFrame)
{
    ...
    range(i, -1, 1) range(j, -1, 1)
    {
        ...
        mat2 D0 = to_mat2(t1.x,t1.y,t1.z,t1.w);
        ...
        float4 aabbX = to_float4_aw(_fmaxf(pos - 0.5f, X0 - K*0.5f), _fminf(pos + 0.5f, X0 + K*0.5f)); //overlap aabb
        float2 center = 0.5f*(swi2(aabbX,x,y) + swi2(aabbX,z,w)); //center of mass
        float2 size = _fmaxf(swi2(aabbX,z,w) - swi2(aabbX,x,y), 0.0f); //only positive
...
__DEVICE__ float2 normalize2(float2 a)
{
    ...
    return (a.x==0.0f && a.y==0.0f) ? to_float2_s(0.0f):normalize(a);
}
...
__KERNEL__ void BuildingSmasherFuse__Buffer_C(float4 U, float2 pos, float4 iMouse)
{
        ...
        if(M > 0.0f)
        {
            range(i, -2,2) range(j, -2, 2)
            {
                if(!(i == 0 && j == 0))
                {
                    ...
                    float2 X0 = 0.0f*DECODE(data.x) + tpos;
                    float2 V0 = DECODE(data.y);
                    float M0 = data.z;
                    float2 dx = X0 - X;
                    float2 dv = V0 - V;
                    mat2 D0 = mat2(T1(tpos));
                    float weight = GS(0.8f*dx);
                   
                    //F += M0*strain((D0*M + D*M0)/(M+M0))*dx*weight;
                    mat2 strain0 = (M0*strain(D0) + M*local_strain) / (M0+M) + to_mat2_f(2.4f*dot(dx,dv));
                    F += M0* mul_mat2(strain0 , dx*weight);
    ...
    //save
    X = X - pos;
    U = to_float4(ENCODE(X), ENCODE(V), data.z, ENCODE(C));
...
vec2 normalize2(float2 a)
{
    return all(equal(a, to_float2_s(0.0f)))?to_float2_s(0.0f):normalize(a);
}

__KERNEL__ void BuildingSmasherFuse__Buffer_D(float4 U, float2 pos)
{
    ...
    if(M > 0.01f) //not vacuum
    {
      ...
      float2 dsize = clamp(destimator(X0 - tpos, data.z), 0.3f, 1.0f);
            float weight = clamp(M0, 0.0f, 1.0f)*GS(0.8f*dx);
            rho += M0*weight;
        ...
        D = D*(1.0f - r) + mat2(1.0f)*r;
        
        //clamp the gradient to not go insane
        D = mat2(clamp(to_float4(D - mat2(1.0f)), -5.0f, 5.0f)) + mat2(1.0f);
    }
...
__KERNEL__ void BuildingSmasherFuse(float4 col, float2 pos, float4 iMouse, sampler2D iChannel2, sampler2D iChannel3)
{
    ...
    range(i, -2, 2) range(j, -2, 2)
    {
      ...
      mat2 D0 = mat2(T1(tpos));
        ...
        rho2 += M0*smoothstep(0.1f, -0.1f, bsdf)/(dsize.x*dsize.y);
    }
   ...
   float d = smoothstep(0.3f,0.7f,_mix(rho, rho2,1.0f));
   swi3(col,x,y,z) = _mix(to_float3_s(0.0f),5.0f*to_float3(0,0.5f,1.0f)*De*De + 0.04f*rho2 + 0.02f*texture(iChannel2, 3.0f*c).xyz, d);
   swi3(col,x,y,z) = _sqrtf(swi3(col,x,y,z));
