...
__KERNEL__ void RhodiumLiquidCarbonFuse__Buffer_A(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, sampler2D iChannel0)
{
  ...
  CONNECT_SLIDER1(Mix2, 0.0f, 1.0f, 0.0f);
  CONNECT_SLIDER2(Threshold, -50.0f, 100.0f, 12.0f);
  CONNECT_SLIDER3(TexBright, -1.0f, 10.0f, 1.0f);
...
else color = swi3(tex,x,y,z) * TexBright;
