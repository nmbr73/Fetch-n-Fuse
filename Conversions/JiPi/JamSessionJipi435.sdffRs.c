...
__DEVICE__ mat3 lookAt(float3 origin, float3 target, float roll) {
  float3 rr = to_float3_aw(_sinf(roll), _cosf(roll), 0.0f);
...
__DEVICE__ float trace(float3 r, float3 o){
    float t = 0.0f;
...
__DEVICE__ float3 getNormal(float3 p){
  ...
  return normalize((to_float3(map(p + swi3(e,y,x,x)),
                           map(p + swi3(e,x,y,x)), 
                           map(p + swi3(e,x,x,y))) - map(p)) / e.y);
}

__KERNEL__ void JamSessionJipi435Fuse(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, sampler2D iChannel0)
{
  ...
  float2 uv = fragCoord / iResolution;
    ...
    float3 ray = normalize(to_float3(uv, 1.0f - dot(uv, uv) * 0.25f));
    float3 o = to_float3_aw(2.0f*_cosf(time), -0.5f*0.75f+_sinf(time/2.0f)*0.75f, 
                  2.0f*_sinf(time));
    ...
    col += to_float3(1.0f-smoothstep(0.0f,0.01f,ray.y-s*LEVELS_SCALAR));
    col *= gradCol;

    ref += to_float3(1.0f-smoothstep(0.0f,-0.01f,ray.y+s*LEVELS_SCALAR));
    ...
    float dither = noise3D(to_float3(swi2(ray,z,y),time))*15.0f/256.0f;
    col += dither;
    ...
    float3 sp = o+ray * hit;
    float d = map(sp);
    float3 n = getNormal(sp);
    
   
    float diff = clamp(dot(n, l), 0.15f, 1.0f);
    
    if(_fabs(d) < 0.05f)
        fragColor = to_float4_aw(diff);
    else
        fragColor = to_float4(col, 1.0f);
