...
//amount of particles
__DEVICE__ const int MAX_PARTICLES = 50000; 
__DEVICE__ const int PARTICLE_INIT_X = 10;
__DEVICE__ const float PARTICLE_SIZE = 0.003f;
__DEVICE__ const float PARTICLE_REPEL_SIZE = 0.005f;
...
#define NUM_WING_POINTS  25
#ifdef ORG
__DEVICE__ float2 WING_POINTS[NUM_WING_POINTS] = {
      to_float2(-0.9f,  0.8f),
      to_float2(-0.65f,  0.5f),
      to_float2(-0.85f,  0.45f),
      to_float2(-0.55f,  0.3f),
      to_float2(-0.75f,  0.125f),
      to_float2(-0.45f,  0.1f),
      to_float2(-0.63f,  -0.2f),
      to_float2(-0.35f, -0.1f),
      to_float2(-0.51f,  -0.45f),
      to_float2(-0.25f, -0.3f),
      to_float2(-0.2f, -0.3f),
      to_float2(-0.1f, -0.4f),
      to_float2(-0.075f, -0.6f),
      to_float2(-0.175f, -0.75f),
      to_float2(-0.05f, -0.7f),
      to_float2(-0.15f, -0.85f),
      to_float2(-0.045f, -0.8f),
      to_float2(-0.05f, -0.9f),
      to_float2(-0.0f, -0.85f),
      to_float2(-0.0f, 0.125f),
      to_float2(-0.045f, 0.0f),
      to_float2(-0.125f, -0.1f),
      to_float2(-0.2f, -0.0f),
      to_float2(-0.3f,  0.2f),
      to_float2(-0.6f,  0.6f)
    };
...
__DEVICE__ float sdPolygonWP( in float2 v[NUM_WING_POINTS], in float2 p )
{
    ...
    for( int i=0, j=NUM_WING_POINTS-1; i<NUM_WING_POINTS; j=i, i++ )
    {
        ...
        d = _fminf( d, dot(b,b) );
        bool c[3] = {p.y>=(v[i]).y, p.y<(v[j]).y, e.x*w.y>e.y*w.x};
        //if( all(c) || all(not(c)) ) s*=-1.0f;
        if( (c[0]==1 && c[1]==1 && c[2]==1) || (c[0]==0 && c[1]==0 && c[2]==0))  s*=-1.0f;
    }
    return s*_sqrtf(d);
}

#define NUM_HEAD_POINTS  9

__DEVICE__ float2 HEAD_POINTS[NUM_HEAD_POINTS] = {
        {-0.0f, -0.2f},
        {0.0f, 0.3f},
        {-0.1f, 0.4f},
        {-0.2f, 0.425f},
...
__DEVICE__ float sdPolygonHP( in float2 v[NUM_HEAD_POINTS], in float2 p )
{
...
__DEVICE__ float sdPhoenix(in float2 p)
{
#if 1
    float minDist = sdPolygonWP(WING_POINTS, p);
    minDist = _fminf(minDist, sdPolygonWP(WING_POINTS, to_float2(-p.x, p.y)));
    minDist = _fminf(minDist, sdPolygonHP(HEAD_POINTS, p));
    return minDist;
#else
    float minDist = sdBox(p, to_float2(0.05f, 0.1f));
    minDist = _fminf(minDist, sdTriangle(p, to_float2(-0.9f, 0.9f), to_float2(-0.0f, -0.0f), to_float2(-0.5f, 0.9f)));
    minDist = _fminf(minDist, sdTriangle(p, to_float2( 0.9f, 0.9f), to_float2(-0.0f, -0.0f), to_float2( 0.5f, 0.9f)));
    return minDist;
#endif
}

__DEVICE__ float2 normPhoenix(in float2 p)
{
    const float DELTA = 0.001f;
    return to_float2(sdPhoenix(p + to_float2(DELTA, 0.0f)) - sdPhoenix(p - to_float2(DELTA, 0.0f)),
                     sdPhoenix(p + to_float2(0.0f, DELTA)) - sdPhoenix(p - to_float2(0.0f, DELTA)));
}

__DEVICE__ float distanceFromWalls(float2 point, float2 resolution, float time)
{
    float minDist = 1e30;
    minDist = _fminf(minDist, point.x + resolution.x / resolution.y);
    minDist = _fminf(minDist, resolution.x / resolution.y - point.x);
...
__KERNEL__ void SparkPhoenixJipiFuse__Buffer_A(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, float4 iMouse, float iTimeDelta, int iFrame)
{
...
float AAAAAAAAAAAAAAA;
    ...
    const float PARTICLE_REPEL = 0.0000005f;
...
//    float4 state = texelFetch(iChannel2, to_int2(0), 0);
    float4 state = texture(iChannel2, (make_float2(to_int2(0,0))+0.5f)/iResolution);
    ...
    if (dataType == POS_VEL)
    {
        if (iFrame == 0 || keyDown || state.z > 0.0f)
        {
          float ffffffffffff;
          if(StartTex)
          {
...
//            data.pos = to_float2(i / particlesPerRow, j / particlesPerRow) * 1.8f - 0.9f;
            data.pos = (to_float2(i / particlesPerRow, j / particlesPerRow) * 1.8f - 0.9f) * to_float2(iResolution.x / iResolution.y, 1.0f);
        ...
        else
        {
            ...
            if (iMouse.z > 0.0f)// && iMouse.w < 0.0f)
            {
                ...
                disturbPos = (to_float2(4.0f, 2.0f) * (swi2(iMouse,x,y) - 0.5f * iResolution) / iResolution.y);
            ...
            float MOUSE_FIELD_STRENGTH = 0.1f; //0.5f / _sqrtf(iFrameRate); // not defined
            ...
            for(int i = 0; i < 4; i++){
                ...
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    data.vel += 0.004f * to_float2(deltaPos.y, -deltaPos.x);
                }
            }       
            ...
            float2 pSpace = data.pos * to_float2(iResolution.y / iResolution.x, 1.0f) * 1.2f;
            if (sdPhoenix(pSpace) >= 0.0f)
            {
                force -= normPhoenix(pSpace) * 0.015f;
            }
            ...
            data.pos = clamp(data.pos, to_float2(-iResolution.x / iResolution.y, -1.0f), to_float2(iResolution.x / iResolution.y, 1.0f));
    ...
    else
    {
        ...
        A2I closest;
        closest.I = fxGetClosest(to_int2_cfloat(world2screen(data.pos * to_float2(iResolution.y / iResolution.x, 1.0f))));
float ttttttttttttttt;        
        int4 bestIds = to_int4(-1,-1,-1,-1);
        float4 bestDists = to_float4_s(1e6);//to_float4(length2(swi2(fxGetParticleData(bestIds.x, POS_VEL),x,y) - data.pos),
...
__DEVICE__ float distance2Particle(int id, float2 fragCoord, float2 iResolution, __TEXTURE2D__ iChannel0){
    if(id==-1) return 1e20;
    float2 delta = (swi2(fxGetParticleData(id, POS_VEL),x,y) * to_float2(iResolution.y / iResolution.x, 1.0f))-fragCoord;
float gggggggggggg;    
    return dot(delta, delta);
}

__KERNEL__ void SparkPhoenixJipiFuse__Buffer_B(float4 fragColor, float2 fragCoord, float2 iResolution, int iFrame)
{
    CONNECT_CHECKBOX0(Reset, 0);
...
float BBBBBBBBBBBBBBBBBBBB;
    ...
    int4 _new = to_int4(-1,-1,-1,-1);//fxGetClosest(to_int2_cfloat(fragCoord));
    //in this vector the distance to these particles will be stored 
    float4 dis = to_float4_s(1e6);//to_float4(distance_f2(swi2(fxGetParticleData(_new.x, POS_VEL),x,y), screen2world(fragCoord)),
    ...
    for(int i = 0; i < 4; i++){
        ...
        for (int j = 0; j < 4; ++j)
        {
            ...
            if (length(otherPos - p.pos) < 0.1f)
            {
                ...
                swi3S(fragColor,x,y,z, swi3(fragColor,x,y,z) + 10.0f*color * _fmaxf(0.0f, PARTICLE_SIZE * 0.3f - _sqrtf(distToLin)) / (PARTICLE_SIZE));            }
        }
    }
    #endif
...
__KERNEL__ void SparkPhoenixJipiFuse__Buffer_C(float4 fragColor, float2 fragCoord, float2 iResolution)
{
    CONNECT_CHECKBOX0(Reset, 0);
...
__KERNEL__ void SparkPhoenixJipiFuse__Buffer_D(float4 fragColor, float2 fragCoord, float2 iResolution, int iFrame, sampler2D iChannel0, sampler2D iChannel1)
{
    ...
    if ((int)fragCoord.x == 0 && (int)fragCoord.y == 0)
    {
        ...
        //float4 state = texelFetch(iChannel1, to_int2(0), 0);
        float4 state = texture(iChannel2, (make_float2(to_int2(0,0))+0.5f)/iResolution);
        
        if (iResolution.x != state.x || iResolution.y != state.y) // (iResolution != swi2(state,x,y))
        {
            state = to_float4(iResolution.x,iResolution.y, 1.0f, 0.0f);
        }
        else
        {
            state.z = 0.0f;
...
__KERNEL__ void SparkPhoenixJipiFuse(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0)
{
