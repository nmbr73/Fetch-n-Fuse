...
__DEVICE__ int2 hash2D(float2 uv) {
  return to_int2_cfloat(_floor(clamp(mod_f2(3e4*sin_f2(mul_f2_mat2((uv) , to_mat2(1,13.51f,73.37f,-57.17f))), (float)(hashEdge)), 0.0f, (float)(hashEdge))));
}
...
__KERNEL__ void BallPitFuse__Buffer_A(float4 state, float2 fragCoord, float2 iResolution, float4 iMouse, int iFrame, sampler2D iChannel0, sampler2D iChannel3)
{
    CONNECT_CHECKBOX0(Reset, 0);
    CONNECT_CHECKBOX1(KeyState, 0);
    ...
    state = texture(iChannel0, (make_float2(ind)+0.5f)/R);
    ...
    if (iMouse.z > 0.0f) {
        ...
        //float keystate = texelFetch(iChannel3, to_int2(69.0f,0),0).x;
        if (KeyState && mousedist < (float)(hashEdge*16)) {
          state = to_float4(-1.0f,-1.0f,0.0f,0.0f);
        //} else if (ind % hashEdge == to_int2(0) && mousedist < float(hashEdge*drawRadius)) {
          } else if ((ind.x % hashEdge == 0 && ind.y % hashEdge == 0 ) && mousedist < (float)(hashEdge*drawRadius)) {
            state = to_float4_f2f2(to_float2_cint(id)*diameter+diameter*0.5f, (noise2D(fragCoord)*2.0f-1.0f)*speed);
        }
    }
    if (iFrame == 0 || Reset) {
        //initialization
        //if (ind % (hashEdge) == to_int2(0,0)) {
        ...
        if ((int)(_floor(state.x/diameter)) != id.x || (int)(_floor(state.y/diameter)) != id.y) state = to_float4(-1.0f, -1.0f, 0.0f, 0.0f);
            ...
            for (int j=-1; j<=1; j++) {
                ...
                for (int k=0; k<hashEdge; k++) {
                    for (int l=0; l<hashEdge; l++) {
                        ...
                        float4 otherstate = texture(iChannel0, (make_float2(otherid*hashEdge + offset)+0.5f)/R);
                        ...
                        if (id2.x == id.x && id2.y == id.y) {
                            for (int h=0; h<numHashes; h++) {
                                ...
                                float2 state0 = swi2(texture(iChannel0, (make_float2(hashInd)+0.5f)/R),x,y);
                                //if (state0 == to_float2(-1.0f)) {
...
#define hardstep(h) (step(to_float2_s(0.0f), h)*2.0f-1.0f)

__KERNEL__ void BallPitFuse__Buffer_B(float4 state, float2 fragCoord, float2 iResolution, sampler2D iChannel0)
{
...
float BBBBBBBBBBBBBBBBBBB;
        ...
        swi2S(state,x,y, swi2(state,x,y) + swi2(state,z,w));
        //bounce off walls
        float2 bounds = diameter*to_float2_cint(particleEdge)-1.0f;
        //if (swi2(state,x,y) != to_float2(-1.0f)) {
          if (state.x != -1.0f || state.y != -1.0f) {
            float2 sgn = hardstep(swi2(state,x,y))*hardstep(bounds-swi2(state,x,y));
            swi2S(state,z,w, swi2(state,z,w) * sgn);
            swi2S(state,z,w, swi2(state,z,w) * _mix(restitution, 1.0f, step(0.0f, sgn.x*sgn.y)));
            swi2S(state,x,y, _fminf(swi2(state,x,y), bounds));
            swi2S(state,x,y, _fmaxf(swi2(state,x,y), to_float2_s(0.0f)));
        }
        float currspeed = length(swi2(state,z,w));
        swi2S(state,z,w, swi2(state,z,w)/currspeed*_fminf(currspeed,maxspeed));
    }
...
__KERNEL__ void BallPitFuse(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0)
{
        ...
        for (int j=-2; j<=2; j++) {
            ...
            for (int k=0; k<hashEdge; k++) {
                for (int l=0; l<hashEdge; l++) {
                    ...
                    float4 state = texture(iChannel0, (make_float2(otherid*hashEdge + offset)+0.5f)/R);
                    ...
                    swi3S(fragColor,x,y,z, swi3(fragColor,x,y,z) + W*to_float3(0.5f, state.z+0.5,state.w+0.5f));
                }
            }
        }
    }
    swi3S(fragColor,x,y,z, swi3(fragColor,x,y,z) / totalW);
