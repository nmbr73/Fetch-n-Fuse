...
__DEVICE__ float scene(float3 p) {
    ...
    float fctr = smoothstep(-0.5f,0.5f,_cosf(bpt/2.0f));
    float fctr2 = smoothstep(0.9f,1.0f,_cosf(bpt/16.0f));
    ...
    d3 = comp(erot(p+10.0f, normalize(to_float3(1,2,3)),1.0f));
...
__KERNEL__ void PartyConFuse(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution)
{
    ...
    t = bp;
  float3 cam = normalize(to_float3_aw(0.8f+_sinf(bp*3.14f/4.0f)*0.3f,uv));
    ...
    for (int i = 0; i <80 && !hit; i++) {
        ...
        if (hit && ((_sinf(d3*45.0f)<-0.4f && (dist!=doodad )) || (dist==doodad && _cosf(pow(length(p2*p2*p2),0.3f)*120.0f)>0.4f )) && dist != lazors) {
            trg = trg || dist==doodad;
            hit = false;
            float3 n = norm(p);
    ...
    float ss = smoothstep(-0.3f,0.3f,scene(p+to_float3_s(0.3f)))+0.5f;
    ...
    if (dl) matcol = _mix(to_float3(1),matcol,0.1f)*0.2f+0.1f;
    float3 col = matcol*fact*ss + _powf(fact,10.0f);
    if (lz) col = to_float3(4);
    ...
    swi3(fragColor,x,y,z) = smoothstep(to_float3_aw(0),to_float3_s(1.2f),swi3(fragColor,x,y,z));
