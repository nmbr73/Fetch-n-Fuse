...
__DEVICE__ float3 sample_grad_dist(float2 uv, float font_size) {
...
__KERNEL__ void KeyboardDebuggingFuse(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0, sampler2D iChannel1)
{
    ...
    if (dbox < 0.0f) {
        ...
        color = ktex[0] ? to_float3(1, 0.25f, 0.25f) : to_float3_s(0.8f);
        ...
        for (int i=0; i<3; ++i) {
            ...
            if (digit > 0 || nonzero || i == 2) {
                ...
                dtext = _fminf(dtext, _fmaxf(max(dbox.x, dbox.y), sample_grad_dist(uv, 1.0f).z));
                nonzero = true;
        ...
        float3 textcolor = ktex[2] ? to_float3_aw(0) : (ktex[1] || ktex[0]) ? to_float3(1) : to_float3_s(0.9f);
