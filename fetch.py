  ...
  for entry in json_data['renderpass']:
    ...
    for input in inputs:
      ...
      if media != None:
        ...
        # known asset
        filename=ASSETS_PATH+media['folder']+"/"+media['name']+"."+media['suffix']

      else:

        # unknown media
        filename=ASSETS_PATH+"UNKNOWN/"+re.sub(r'/','_',src)
...
def do_tst(shader_id):
  print(f"Test '{shader_id}'")
  print(f"Assets={NOASSETS}")
