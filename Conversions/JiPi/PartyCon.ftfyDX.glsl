...
float scene(vec3 p) {
    ...
    float fctr = smoothstep(-.5,.5,cos(bpt/2.));
    float fctr2 = smoothstep(.9,1.,cos(bpt/16.));
    ...
    d3 = comp(erot(p+10., normalize(vec3(1,2,3)),1.));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    cam = erot(cam,vec3(0,1,0),cos(bp*.2)*.4);
    cam = erot(cam,vec3(0,0,1),sin(bp*.2)*.4);
    vec3 p = init;
    bool hit = false;
    float atten = 1.;
    float tlen = 0.;
    ...
    for (int i = 0; i <80 && !hit; i++) {
        ...
        if (hit && ((sin(d3*45.)<-0.4 && (dist!=doodad )) || (dist==doodad && cos(pow(length(p2*p2*p2),.3)*120.)>.4 )) && dist != lazors) {
            trg = trg || dist==doodad;
            hit = false;
            vec3 n = norm(p);
