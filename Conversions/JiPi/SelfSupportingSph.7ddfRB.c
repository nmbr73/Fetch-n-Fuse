...
//amount of particles
__DEVICE__ const int MAX_PARTICLES = 3000; 
__DEVICE__ const int PARTICLE_INIT_X = 10;
__DEVICE__ const float PARTICLE_SIZE = 0.03f;
__DEVICE__ const float PARTICLE_REPEL_SIZE = 0.015f;
...
__DEVICE__ fxParticle fxGetParticleInternal(__TEXTURE2D__ sampler, int resolutionWidth, int id, float2 iResolution)
{
    ...
    float4 particleData5 = texture(sampler, (make_float2(fxLocFromIDInternal(resolutionWidth, id, FLUID))+0.5f)/iResolution);
...
{    
    ...
    case FLUID:
        return to_float4(p.uv.x,p.uv.y, p.density, p.pressure);
    }
...
__KERNEL__ void SelfSupportingSphFuse__Buffer_A(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, float4 iMouse, float iTimeDelta, int iFrame)
{
    ...
    const float2 GRAVITY = to_float2(0.0000f, -0.00002f);
    ...
    if (dataType == POS_VEL || dataType == FLUID)
    {
        ...
        else
        {
            ...
            float MOUSE_FIELD_STRENGTH = 0.1f; // 0.1f / _sqrtf(iFrameRate); // not defined
...
__KERNEL__ void SelfSupportingSphFuse__Buffer_B(float4 fragColor, float2 fragCoord, float2 iResolution, int iFrame)
{
    ...
    for(int _x=-2; _x<=2; _x++){
        for(int _y=-2; _y<=2; _y++){
            //int4 old   = fxGetClosest( iFragCoord + to_int2( _x, _y) );      
            A2I old; 
...
__DEVICE__ const float PARTICLE_SDF_SIZE = 0.04f;
...
{   
    ...
    float3 color = to_float3(200.0f*p.pressure, 30000.0f*dot(p.vel, p.vel), 0.000012f*p.density*p.density*p.density);
...
float rrrrrrrrrrrrrrrrrrrrr;
            if (length(otherPos - p.pos) < 0.1f)
            {
                ...
                swi3S(*fragColor,x,y,z, swi3(*fragColor,x,y,z) + color * _fmaxf(0.0f, PARTICLE_SIZE * 0.3f - _sqrtf(distToLin)) / (PARTICLE_SIZE));
            }
        }
    }
    #endif
...
__KERNEL__ void SelfSupportingSphFuse(float4 fragColor, float2 fragCoord, float2 iResolution)
{
