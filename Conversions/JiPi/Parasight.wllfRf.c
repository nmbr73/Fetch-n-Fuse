...
#define fluid_rho 0.00752f


//mold stuff 
#define sense_ang 0.862f
#define trailing 1.0f*_cosf(time*01.21f)
#define sense_dis 04.0f
#define sense_force 0.500f
#define acceleration 0.01f*(time*0.2f)


//SPH stuff
__DEVICE__ float Pf(float2 rho, float time)
{
    return 0.15f*rho.x + 0.0f*rho.y; //gas
    return(0.052f*_cosf(time*01.21f))*rho.x*(rho.x/fluid_rho - 1.0f); //water pressure
}
...
//diffusion amount
#define dif 01.30f
__DEVICE__ float3 distribution(float2 _x, float2 p, float K)
{
    float4 aabb0 = to_float4_f2f2(p - 0.5f, p + 0.5f);
    float4 aabb1 = to_float4_f2f2(_x - K*0.5f, _x + K*0.5f);
...
__DEVICE__ particle Simulation(__TEXTURE2D__ ch, particle P, float2 pos, float time, float2 R, float4 Mouse)
{
    ...
    //gravity
    F -= sin_f2(-0.000300f*P.M.x*to_float2(0,1));
    ...
    //velocity limit
    float v = length(P.V);
    P.V /= (v > 2.0f * _sinf(time*0.61f))?1.0f*v:1.0f;
...
__KERNEL__ void ParasightFuse__Buffer_A(float4 U, float2 pos, float2 iResolution, float iTime, float4 iMouse, int iFrame)
{
    ...
    CONNECT_BUTTON0(Modus, 1, X,  V1, V2, M, Special);
    ...
    if (Blend1 > 0.0f)
    {
      ...
      {      
        ...
        if ((int)Modus&4)
        {
          P.V =  _mix( P.V, 0.5f*(swi2(rand,x,y)-0.5f) + 0.0f*to_float2(_sinf(2.0f*pos.x/R.x), _cosf(2.0f*pos.x/R.x)), Blend1);
        }          


        if ((int)Modus&8)
        {  
          P.V =  _mix( P.V, (swi2(tex,x,y)-BlendOff)*BlendMul, Blend1);
        }

        if ((int)Modus&16) //Masse
          P.M = _mix(P.M, Par, Blend1);

        if ((int)Modus&32) //Special
        {
          P.X = _mix(P.X, to_float2_s(0.0),Blend1);
        }
      }
    }
...
//        float3 rand = hash32(pos);
        ...
        float4 tex = to_float4_s(0.0f);
        
        if (Textur)
        {
            tex = texture(iChannel1, pos/iResolution);
            if (tex.w>0.0f)
            {
              P.X = pos;
...
__KERNEL__ void ParasightFuse__Buffer_B(float4 U, float2 pos, float iTime, float2 iResolution, float4 iMouse)
{
    pos+=0.5f;
    ...
    {
        P = Simulation(ch0, P, pos, iTime, iResolution, iMouse);
    }
...
__KERNEL__ void ParasightFuse__Buffer_C(float4 fragColor, float2 pos, float iTime, float2 iResolution)
{
    pos+=0.5f;
...
__DEVICE__ float3 hsv2rgb( in float3 c )
{
  ...
  rgb = rgb*rgb*(2.0f-3.0f*rgb); // cubic smoothing  
...
__KERNEL__ void ParasightFuse(float4 col, float2 pos, float iTime, float2 iResolution)
{
    pos+=0.5f;
    ...
    float4 rho = V(ch1,pos, iResolution);
    float3 dx = to_float3(-3.0f, 0.0f, 3.0f);
    float4 grad = -0.5f*to_float4_f2f2(swi2(V(ch1, pos + swi2(dx,z,y), iResolution),z,w) - swi2(V(ch1, pos + swi2(dx,x,y), iResolution),z,w),
                                       swi2(V(ch1, pos + swi2(dx,y,z), iResolution),z,w) - swi2(V(ch1, pos + swi2(dx,y,x), iResolution),z,w));
    ...
    float3 col0 = to_float3(1.0f, 0.7f, 0.7f);
    float3 col1 = to_float3(0.0f, 0.92f, 1.0f);
    
    // Output to screen
    float3 colxyz = swi3(col,x,y,z) + to_float3_s(0.1f*a);
           colxyz += 0.5f - 0.5f*cos_f3(0.51f*0010.0f*to_float3(0.52f*_sinf(iTime*0.11f),0.532f*_cosf(iTime*0.12f),0.42f*_cosf(iTime*(00.13f)))*rho.w);
    //swi3(col,x,y,z) += to_float3(1,1,1)*bord;
    
          colxyz = tanh_f3(4.0f*pow_f3(colxyz,to_float3_s(1.5f)));
