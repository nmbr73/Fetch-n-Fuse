...
{
  return fract(_sinf(dot(co*0.123f,to_float2(12.9898f,78.233f))) * 43758.5453f);
}
...
float normalizer = 1.0f / _sqrtf(1.0f + nudge*nudge);  // pythagorean theorem on that perpendicular to maintain scale
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        swi2(p,x,y) += to_float2(p.y, -p.x) * nudge;
        swi2(p,x,y) *= normalizer;
        // rotate on other axis
        swi2(p,x,z) += to_float2(p.z, -p.x) * nudge;
        swi2(p,x,z) *= normalizer;
...
{
  #ifdef ROTATION
  R(swi2(p,x,z), iMouse_x*0.008f*pi+iTime*0.1f);
  #endif
...
#define R(p, X,Y, a) p=_cosf(a)*to_float2(p.X,p.Y)+_sinf(a)*to_float2(p.Y, -p.X)
  ...
  sum *= 1.0f / _expf( ld * 0.2f ) * 0.6f;
    ...
    if (td<0.8f)
    {
        ...
        starbg = _mix(starbg, to_float3(0.8f,0.9f,1.0f), smoothstep(0.99f, 1.0f, stars)*clamp(dot(to_float3_s(0.0f),rd)+0.75f,0.0f,1.0f));
