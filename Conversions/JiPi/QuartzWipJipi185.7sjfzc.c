...
__DEVICE__ float3 hash33_(float3 p){     
    ...
    return fract(to_float3(2097152, 262144, 32768)*n); 
}
#define M0 1597334673U
#define M1 3812015801U
#define M2 uto_float2(M0, M1)
#define M3 uto_float3(M0, M1, 2798796413U)
...
__DEVICE__ float hash12( float2 p ) { uvec2 q = uto_float2(to_int2(p)) * M2; uint n = (q.x ^ q.y) * M0; return float(n) * (1.0f/float(0xffffffffU)); }
__DEVICE__ float3 hash33(float3 p) { uvec3 q = uto_float3(to_int3(p)) * M3; q = (q.x ^ q.y ^ q.z)*M3; return to_float3(q) * (1.0f/float(0xffffffffU)); }
...
{
    float2 i = _floor( p ), f = fract( p ), u = f*f*f*(3.0f-2.0f*f);
...
__DEVICE__ float noiseFF(in float2 uv) {
   ...
   mat2 m = mat2( 1.6f,  1.2f, -1.2f,  1.6f ) * 1.25f;
    
    float f  = 0.5f*noiseF( uv ); uv = m*uv;
    f += 0.2500f*noiseF( uv ); uv = m*uv;
    f += 0.1250f*noiseF( uv ); uv = m*uv;
    f += 0.0625f*noiseF( uv ); uv = m*uv;   
...
F n(in float3 p) {
    ...
    W   ii = swi2(i,x,y) + i.z * 5.0f + 5.0f;
    
    #define II(a,b) H(swi2(i,x,y) + i.z * W(5.0f) + W(a,b))
    
    F   v1 = _mix(mix(II(Z,Z),II(J,Z),u.x), _mix(
            II(Z,J),II(J,J),u.x), u.y);
    
    #define I2(a,b) H(ii + W(a,b))
    return _fmaxf(_mix(v1,_mix(mix(I2(Z,Z),I2(J,Z),u.x), 
            _mix(I2(Z,J),I2(J,J),u.x), 
            u.y),u.z),Z);
}
...
__DEVICE__ float2 sgn(float2 v) {
  return to_float2((v.x<0.0f)?-1.:1., (v.y<0.0f)?-1.:1.);
}
...
__DEVICE__ float2 pModMirror2(inout float2 p, float2 size) {
  ...
  p = mod_f(p + halfsize, size) - halfsize;
  p *= mod_f(c,to_float2(2))*2.0f - to_float2_s(1.0f);
  return c;
}
...
__DEVICE__ float fBox(float3 p, float3 b) {
  float3 d = _fabs(p) - b;
  return length(_fmaxf(d, to_float3_aw(0))) + vmax(_fminf(d, to_float3(0)));
}

// Same as above, but in two dimensions (an endless box)
__DEVICE__ float fBox2Cheap(float2 p, float2 b) {
  return vmax(_fabs(p)-b);
}
...
{
    return mat2x2(_cosf(angle), -_sinf(angle),
...
__DEVICE__ float3 sd2d(float2 p, float o)
{
   ...
   float s =0.45f, d, d2 = 1.0f, d3= 0.0f;
    ...
    p = p*rot((mod_f(timeline.patternNum, 4.0f) > 1.0f ? -1.0f : 1.0f) * -0.9f * time);// twist

    for ( i = 0; i<NIFS; i++)
    {        
        if (p.x < 0.0f) { p.x = -p.x; col.x++;}
    //p = p*rot(0.1f*_sinf(time));
...
__DEVICE__ float3 opTwist(in float3 p, float k)
{
    ...
    float s = _sinf(k*p.y);
    mat2  m = mat2(c,-s,s,c);
    float3  q = to_float3(m*swi2(p,x,z),p.y);
...
__DEVICE__ float3 discoBall(float3 p, float spikes) {
    ...
    float pxz = pModPolar(swi2(p,x,z), 7.0f);//_ceil(vol2 * 10.0f));// * hash12(to_float2(_floor(part2Time * 3.55f))) * _fminf(part2Time, 30.0f));
    float pyz = pModPolar(swi2(p,x,y), 7.0f);
...
geometry map(float3 p) {
  ...
  //p.x *= _sinf(timeline.smoothNPatternTime + p.x);
    ...
    pR(swi2(bp,x,y), timeline.songTime * 0.02f + n(to_float3(iTime * 0.1f)) * 5.0f);
    //pModPolar(swi2(p,x,y), 4.0f);
    //pModPolar(swi2(p,x,z), 4.0f);
    float3 tb = bp;
    float3 reps = to_float3(
        pModInterval1(bp.x, 5.0f, -2.0f, 2.0f),
        pModInterval1(bp.y, 5.0f, -2.0f, 2.0f),
        pModInterval1(bp.z, 5.0f, -2.0f, 2.0f)
    );
    ...
    float3 reps2 = to_float3(
        pModInterval1(bp.x, 5.0f, -1.5f, 1.5f),
        pModInterval1(bp.y, 5.0f, -1.5f, 1.5f),
        pModInterval1(bp.z, 5.0f, -1.5f, 1.5f)
    );
  ...
  box.diffuse = 4.0f;
   ...
   // box.dist = _fminf(box.dist, -(length(bp2) -140.0f));
    ...
    pR(swi2(bp,z,y), _sinf(timeline.songTime * 0.03f));
    pModPolar(swi2(bp,x,y), 5.0f);
    box.dist = discoBall(bp * 2.0f, 25.0f).x;
    bp = bp2;
    bp.x += _sinf(timeline.songTime * 0.02f) * 20.0f;
    bp.y += _cosf(timeline.songTime * 0.02f) * 20.0f;
    pR(swi2(bp,x,y), _sinf(timeline.songTime * 0.1f));
    pR(swi2(bp,z,y), _sinf(timeline.songTime * 0.1f));
...
geometry trace(float3 o, float3 d, int maxI) {
    float omega = 0.3f;
    float t = 0.01f;
    ...
    float functionSign = map(o).dist < 0.0f ? -1.0f : +1.0f;
...
__KERNEL__ void QuartzWipJipi185Fuse__Buffer_A(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, float iChannelTime[], sampler2D iChannel0, sampler2D iChannel1)
{


    timeline.songTime = texelFetch(iChannel0, to_int2(0, 0), 0).r;
    timeline.rPatternTime = texelFetch(iChannel0, to_int2(0, 0), 0).g;
    timeline.nPatternTime = texelFetch(iChannel0, to_int2(0, 0), 0).b;
    timeline.smoothNPatternTime = texelFetch(iChannel0, to_int2(0, 0), 0).a;
    
    timeline.patternNum = texelFetch(iChannel0, to_int2(1, 0), 0).x;
    
    float3 campos = texelFetch(iChannel0, to_int2(5, 0), 0).rgb;
    sdfMixer.grid = texelFetch(iChannel0, to_int2(11, 0), 0).r;
    sdfMixer.star = texelFetch(iChannel0, to_int2(13, 0), 0).g;
    sdfMixer.ifs1 = texelFetch(iChannel0, to_int2(15, 0), 0).r;
    ...
    float rr = timeline.songTime + (n(to_float3(timeline.songTime * 0.4f)) * 2.0f) - 0.5f;
    float3 
        vuv = to_float3(0.0f, 1.0f, 0.4f ), // up
      
        ro = campos;
    
    float3
        vrp =  to_float3(0.0f, 0.0f, 0.0f),//+ ro,
  ...
  float3 sky = Sky(rd, true, normalize(light), ro);
   ...
   sceneColor *= ao; 
    ...
    float3 bsceneColor = texture(iChannel1, (fragCoord + to_float2(1.0f, 0.0f)) / iResolution).rgb;
...
// Connect Buffer B 'https://soundcloud.com/neurofunksocietyofpoland/dronehead-02-hexyll-original-mix' to iChannel0


vec3 U[2] = vec3[2](to_float3_aw(0), to_float3(0));

__DEVICE__ void compute(in int2 coord, out float4 color) {
    float time = U[0].x,
    ...
    switch (coord.x) {
        ...
        case 0:
          color = to_float4_aw(U[1].x, (PATTERN_TIME - pt) / PATTERN_TIME, npt, smoothstep(0.0f, 1.0f, pt / PATTERN_TIME));
        ...
        case 5: 
          // RG - cam position 
          float2 x = texture(iChannel1, to_float2(0.5f, fract(time * 0.2f))).rg * 0.4f * npt;
        ...
        case 11:
          /// sdf mixer GRID,
          color = to_float4_aw(pt, _sinf(pt), 0.0f, 0.0f);
          break;
        case 13: 
          break;
        ...
        case 15:
          /// sdf mixer IFS1
          color = to_float4_aw(_fmaxf(0.0f, 1.0f- pt2));
          break;
    }
}

__KERNEL__ void QuartzWipJipi185Fuse__Buffer_B(float4 fragColor, float2 fragCoord, float iTime, int iFrame, float iChannelTime[], sampler2D iChannel1)
{

  U[1].x = float(iFrame);
    U[0].x = _fmaxf(22.0f, iChannelTime[0]);
    
    int2 store = to_int2(fragCoord);
  float2 uv = swi2(gl_FragCoord,x,y) / U[0].yz;   
...
__DEVICE__ float2 hash( float2 p ) { p=to_float2(dot(p,to_float2(127.1f,311.7f)),dot(p,to_float2(269.5f,183.3f))); return fract(_sinf(p)*18.5453f); }

// return distance, and cell id
__DEVICE__ float2 voronoi( in float2 x )
{
    float2 n = _floor( x );
    float2 f = fract( x );
...
mat2 rot = mat2(_cosf(2.399f),_sinf(2.399f),-_sinf(2.399f),_cosf(2.399f));

__DEVICE__ float3 dof(sampler2D tex,float2 uv,float rad)
{
  float3 acc=to_float3_aw(0);
...
//-------------------------------------------------------------------------------------------
__KERNEL__ void QuartzWipJipi185Fuse(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, sampler2D iChannel0, sampler2D iChannel1)
{

    res = iResolution;
  float2 uv = swi2(gl_FragCoord,x,y) / swi2(res,x,y);
    ...
    float sh = (cell / 50.0f) * texelFetch(iChannel1, to_int2(9, 0), 0).r;//cell > 0.2f ? c.y * 0.5f : 0.0f;
    uv.x += sh * 3.0f;//* hash(uv).x * 2.2f;
    
    float a = _fabs(0.7f - _powf(_tex2DVecN(iChannel0,uv.x,uv.y,15).w * 1.2f, 1.4f));
  fragColor=to_float4(dof(iChannel0,uv,a),1.0f);
    fragColor = _mix(fragColor, _powf(
        _fmaxf(
            to_float4_s(0.0f), 
