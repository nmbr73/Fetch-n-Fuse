...
#define sense_num 11
#define sense_ang 01.5f*(01.5f*_cosf(iTime/6.0f))
//#define sense_dis 12.0f*(010.51f*_cosf(iTime*0.51f))
#define sense_dis 50.0f+(040.0f*_cosf(iTime/3.0f))
#define sense_oscil 00.0f
#define oscil_scale 0.421f
#define oscil_pow 0.50f
#define sense_force 0.01f
#define distance_scale 00.59f
...
__KERNEL__ void RipleysBelieveItFuse__Buffer_A(float4 U, float2 pos, float2 iResolution, int iFrame, sampler2D iChannel0)
{
...
__KERNEL__ void RipleysBelieveItFuse__Buffer_B(float4 U, float2 pos, float2 iResolution, float iTime, float4 iMouse, sampler2D iChannel0, sampler2D iChannel1, sampler2D iChannel2)
{
    ...
    if(M != 0.0f) //not vacuum
    {
        ...
        range(i, -sense_num, sense_num)
        {
          ...
          float2 dir = (1.0f + sense_dis*_powcf(M, distance_scale))*Dir(cang);
          float3 s0 = swi3(C(X + dir),x,y,z);  
          float fs = _powcf(s0.z, force_scale);
          float os = oscil_scale*_powcf(s0.z - M, oscil_pow);
...
__KERNEL__ void RipleysBelieveItFuse__Buffer_C(float4 fragColor, float2 pos, float2 iResolution, sampler2D iChannel0)
{
    pos+=0.5f;  
...
__KERNEL__ void RipleysBelieveItFuse__Buffer_D(float4 fragColor, float2 pos, float2 iResolution, int iFrame, sampler2D iChannel0, sampler2D iChannel1, sampler2D iChannel2)
{
    CONNECT_CHECKBOX0(Reset, 0);
    pos+=0.5f;
...
// Connect Image 'Cubemap: Forest Blurred_0' to iChannel4
...
{
    float y = 1.0f*rho(pos,R,iChannel0);  
...
__KERNEL__ void RipleysBelieveItFuse(float4 col, float2 pos, float2 iResolution, float iTime, float4 iMouse, sampler2D iChannel0, sampler2D iChannel1, sampler2D iChannel2, sampler2D iChannel3)
{
...
float IIIIIIIIIIIIIIIIIIIIIIII;    
        ...
        if(X.w < min_d)
        {
            ...
            float3 colA = swi3(decube_f3(iChannel4,  swi3(rd,y,z,x)),x,y,z);
            float3 colB = (to_float3_s(0.5f) + 0.5f*dot(rd, normalize(to_float3_s(1.0f))));
            colB += 3.0f*rough*_powcf(_fmaxf(dot(rd, normalize(to_float3_s(1.0f))), 0.0f), 10.0f);
            colB += 3.0f*rough*_powcf(_fmaxf(dot(rd, normalize(to_float3(-0.5f,-0.9f,0.8f))), 0.0f), 10.0f);
            float b = clamp(0.5f + 0.5f*dot(n, normalize(to_float3(1,1,1))), 0.0f,1.0f);
            float K = 1.0f - _powcf(_fmaxf(dot(n,rd),0.0f), 2.0f);
            swi3S(col,x,y,z, 1.0f*albedo*colB + 0.0f*rough*colA*K);
        }
        else
        {    
            //background
            col = 1.0f*decube_f3(iChannel4,  swi3(ray,y,z,x));
        }
