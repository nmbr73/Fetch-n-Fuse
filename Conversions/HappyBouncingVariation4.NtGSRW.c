...
__DEVICE__ float2 animation(float2 p, float t)
{
    ...
    p.y -= 0.15f; // sidebar
    p = mul_f2_mat2(p,rot(_sinf(ta)*0.2f*ss(0.0f,0.5f,p.y+0.2f))); // swing
...
__KERNEL__ void HappyBouncingVariation4Fuse(float4 color, float2 pixel, float iTime, float2 iResolution)
{
