...
#define Bi(p) make_int2(mod_f2f2(p,R))
...
#define fluid_rho 0.0200f


//mold stuff 
#define sense_ang 0.1764325975283464746f
#define sense_dis 0.05f/(time*0.30f)+03.0f
#define sense_force 0.4328f
#define trailing 02.10f/(time*03.0f)
#define acceleration 0.0751f

//SPH stuff
__DEVICE__ float Pf(float2 rho, float time)
{
    return 0.05f/(time*0.30f)/01.042347503666f*rho.x + 0.0f*rho.y; //gas
...
//diffusion amount
#define dif 0.5f*_sinf(time*02.240f)+01.1f
...
__DEVICE__ particle Reintegration(__TEXTURE2D__ ch, particle P, float2 pos, float time, float2 R)
{
    P.X *= 0.0f;
    P.V *= 0.0f;
    P.M.x *= 0.0f;
...
__DEVICE__ particle Simulation(__TEXTURE2D__ ch, particle P, float2 pos, float time, float4 Mouse, float2 R)
{
    ...
    range(i, -2, 2) range(j, -2, 2)
    {
        ...
        particle P0 = getParticle(data, tpos);
        float2 dx = P0.X - P.X;
        float avgP = 0.5f*P0.M.x*(Pf(P.M,time) + Pf(P0.M,time)); 
...
__KERNEL__ void InorganicaPermutationsFuse__Buffer_A(float4 U, float2 pos, float iTime, float2 iResolution, float4 iMouse, int iFrame)
{
    CONNECT_CHECKBOX0(Reset, 0);
    
    CONNECT_SLIDER3(Blend2, 0.0f, 1.0f, 0.0f);
    CONNECT_SLIDER4(Blend2V, -100.0f, 100.0f, 1.0f);
    CONNECT_SLIDER5(Blend2M, -100.0f, 100.0f, 1.0f);
//    CONNECT_SLIDER3(BlendM_Thr, -10.0f, 10.0f, 0.0f);
    CONNECT_BUTTON1(Modus2, 1, XY,  V, M, Clear, BufA);
    
    pos+=0.5f;

    float2 R = iResolution; //time = _sinf(iTime); Mouse = iMouse;
    ...
    P = Reintegration(ch0, P, pos, iTime, iResolution);
    ...
    {
      //float2 tuv = U/R;
      float4 tex = pixel(ch1,pos);

      if (tex.w > 0.0f)
      {      
        if ((int)Modus2&2)
          P.X = _mix(P.X,pos, Blend2); // Position

        if ((int)Modus2&4)
          P.V = _mix(P.V, swi2(tex,x,y)*Blend2V-Blend2V/2.0f, Blend2);

        if ((int)Modus2&8)
        {  
          P.M = _mix(P.M, swi2(tex,x,y)*Blend2M-Blend2V/2.0f, Blend2);
        }
...
__KERNEL__ void InorganicaPermutationsFuse__Buffer_B(float4 U, float2 pos, float iTime, float2 iResolution, float4 iMouse)
{

    CONNECT_CHECKBOX0(Reset, 0);
    
    CONNECT_SLIDER0(Blend1, 0.0f, 1.0f, 0.0f);
    CONNECT_SLIDER1(BlendV, -10.0f, 10.0f, 1.0f);
    CONNECT_SLIDER2(BlendM, -10.0f, 10.0f, 1.0f);
//    CONNECT_SLIDER3(BlendM_Thr, -10.0f, 10.0f, 0.0f);
    CONNECT_BUTTON0(Modus, 1, XY,  V, M, Clear, BufB);


    pos+=0.5f;
    float2 R = iResolution; //time = iTime; Mouse = iMouse;
    int2 p = to_int2_cfloat(pos);
    ...
    {
        P = Simulation(ch0, P, pos, iTime, iMouse,iResolution);
    }
    ...
    if (Blend1 > 0.0f)
    {
      ...
      {      
        if ((int)Modus&2)
          P.X = _mix(P.X,pos, Blend1); // Position

        if ((int)Modus&4)
          P.V = _mix(P.V, swi2(tex,x,y)*BlendV-BlendV/2.0f, Blend1);

        if ((int)Modus&8)
        {  
          P.M = _mix(P.M, swi2(tex,x,y)*BlendM-BlendV/2.0f, Blend1);
        }
...
__KERNEL__ void InorganicaPermutationsFuse__Buffer_C(float4 fragColor, float2 pos, float iTime, float2 iResolution)
{
    pos+=0.5f;

    float2 R = iResolution; //time = _cosf(iTime);
    int2 p = to_int2_cfloat(pos);
...
__DEVICE__ float3 hsv2rgb( in float3 c )
{
  ...
  rgb = rgb*rgb*(3.0f-02.0f*rgb); // cubic smoothing  
...
__KERNEL__ void InorganicaPermutationsFuse(float4 col, float2 pos, float iTime, float2 iResolution)
{
    ...
    pos+=0.5f;
    float2 R = iResolution; //time = iTime;
    //pos = R*0.5f + pos*0.1f;
    int2 p = to_int2_cfloat(pos);
    ...
    //border render
    float3 Nb = bN(P.X, iTime, iResolution);
    float bord = smoothstep(2.0f*border_h,border_h*0.5f,border(pos,iTime, iResolution));
    ...
    float3 col0 = to_float3(1.1f*(iTime*0.3f), 0.7f, 0.7f);
    float3 col1 = to_float3(0.5f, 0.92f, 0.1f*_sinf(iTime*0.3f));
    // Output to screen
    //swi3(col,x,y,z) += 0.1f*a;
    col.x += 0.1f*a;
    col.y += 0.1f*a;
    col.z += 0.1f*a;

    swi3S(col,x,y,z, swi3(col,x,y,z) + 0.5f - 0.5f*cos_f3(1.5f*01.0f*to_float3(0.1f*_sinf(iTime*0.3f),0.1f*_cosf(iTime*0.5f),0.1f*_sinf(iTime*(0.4f)))*rho.w));
    //swi3(col,x,y,z) += to_float3(1,1,1)*bord;
    swi3S(col,x,y,z, tanh_f3(4.0f*pow_f3(swi3(col,x,y,z),to_float3_s(1.5f))));
    
    col = to_float4_aw(swi3(col,x,y,z)+(swi3(Color,x,y,z)-0.5f)*a,a);
    col.w=a;//1.5f;
