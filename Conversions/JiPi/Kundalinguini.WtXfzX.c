...
//mold stuff 
#define sense_ang (time*0.34f)
#define sense_dis 010.0f*_sinf(time*01.1f)
#define sense_force 0.1f
#define trailing 0.0f
#define acceleration 0.0521f


//SPH stuff
__DEVICE__ float Pf(float2 rho)
{
    return 0.75f*rho.x + 0.0f*rho.y; //gas
...
__DEVICE__ particle Simulation(__TEXTURE2D__ ch, inout particle P, float2 pos, float2 R, float time, float4 Mouse)
{
...
__DEVICE__ particle Blending( __TEXTURE2D__ channel, float2 uv, particle P, float Blend, float2 Par, float2 MulOff, int Modus, float2 fragCoord, float2 R, float MAX_SPEED)
{
    ...
    if (Blend > 0.0f)
    {
      ...
      float3 rand = hash32(fragCoord);
      if (tex.w > 0.0f)
      {   
        if ((int)Modus&2) //Startbedingung
        {
          //float q = 2.0f*PI * hash32(1.0f + fragCoord).z;
          
          P.X = _mix(P.X, fragCoord, Blend);
          //P.V = _mix(P.V, MAX_SPEED * to_float2(_cosf(q), _sinf(q)), Blend);
          P.V = _mix(P.V, 0.5f*(swi2(rand,x,y)-0.5f) + 0.0f*to_float2(_sinf(2.0f*fragCoord.x/R.x), _cosf(2.0f*fragCoord.x/R.x)), Blend);
          //P.M = _mix(P.M, 0.45f - _fabs(fragCoord.x/iResolution.x - 0.5f), Blend);
          P.M = _mix(P.M, to_float2(mass, 0.0f), Blend);
...
__KERNEL__ void KundalinguiniFuse__Buffer_A(float4 U, float2 pos, float iTime, float2 iResolution, float4 iMouse, int iFrame)
{
...
__KERNEL__ void KundalinguiniFuse__Buffer_B(float4 U, float2 pos, float iTime, float2 iResolution, float4 iMouse)
{
    pos+=0.5f;
    ...
    {
        P = Simulation(ch0, P, pos,R,time,iMouse);
    }
...
__KERNEL__ void KundalinguiniFuse__Buffer_C(float4 fragColor, float2 pos, float iTime, float2 iResolution)
{
    pos+=0.5f;

    float time = _cosf(iTime);
    int2 p = to_int2_cfloat(pos);
...
__KERNEL__ void KundalinguiniFuse(float4 col, float2 pos, float iTime, float2 iResolution)
{
    pos+=0.5f;
    ...
    //border render
    float3 Nb = bN(P.X, time, R);
    float bord = smoothstep(2.0f*border_h,border_h*0.5f,border(pos,time,R));
    ...
    // Output to screen
    swi3S(col,x,y,z, swi3(col,x,y,z) + 0.1f*a);
    swi3S(col,x,y,z, swi3(col,x,y,z) + 0.5f - 0.5f*cos_f3(0.31f*010.0f*to_float3(0.36131f*_sinf(iTime*0.1f),0.22f+_cosf(iTime*0.1f),0.1606f+_cosf(iTime*(0.51f)))*rho.w));
    //swi3S(col,x,y,z, swi3(col,x,y,z) += to_float3(1,1,1)*bord;
