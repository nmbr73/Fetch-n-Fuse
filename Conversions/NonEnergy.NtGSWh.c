...
__DEVICE__ float3 texTiedCylinder(float2 uv, __TEXTURE2D__ iChannel0) {
    ...
    float3 tex = swi3(_tex2DVecN(iChannel0,newUv.x,newUv.y,15),x,y,z);
    return to_float3_aw(swi2(tex,x,y) * velScalar, tex.z);
}
...
__KERNEL__ void NonEnergyFuse__Buffer_A(float4 fragColor, float2 fragCoord, float iTime, float2 iResolution, float4 iMouse, int iFrame, sampler2D iChannel0)
{
  ...
  // get the 3x3 neighborhood.
    ...
    float2 newFlow = swi2(sum,x,y) + transfer * transferAmount;
    
    // initialize
    if (length(swi2(iMouse,z,w) - fragCoord) < 25.0f) {
        ...
        fragColor = to_float4(rand(vUv).x * 10.0f,rand(vUv).y * 10.0f, 0.0f, 1.0f);
    } else {
        float max = 100.0f;
...
__KERNEL__ void NonEnergyFuse(float4 fragColor, float2 fragCoord, float2 iResolution, sampler2D iChannel0)
{

    // Normalized pixel coordinates (from 0 to 1)
    float2 uv = fragCoord/iResolution;

    float3 values = swi3(_tex2DVecN(iChannel0,uv.x,uv.y,15),x,y,z);
    
    float2 vector = swi2(values,x,y);
